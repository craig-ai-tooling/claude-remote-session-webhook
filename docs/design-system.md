...
| `.quota-label` | `flex-basis: 100%` + `order: 1` | The weekly-quota words have no room on the line the brand and the identity share, so they take their own line below both. `order` moves the display only; in the markup they still follow the brand, where a desktop shows them |

...
// default.
//
// "Sign-in" used to be a second name shownSection recognised on purpose,
// beside Updates — the section the sign-in relay's panel lived on. Spec 015
// moved that panel behind the header's own auth control and deleted the
// section entirely, and deliberately added no special case here to take its
// place: `?section=Sign-in` is now a value nothing recognises, which is
// exactly the "somebody kept an old link" case this function already handles.
